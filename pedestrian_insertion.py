...
def crop_and_save(image, bbs, crop_size, counter, save_dir):
    '''
    ...
    bbs_img = []
    for bb in bbs:
        x, w = int(bb[0]*img_w), int(bb[2]*img_w)
        y, h = int(bb[1]*img_h), int(bb[3]*img_h)
        bb_img = [x,y,w,h]
        bbs_img.append(bb_img)

    # crop image
    for x,y,w,h in bbs_img:
        bb_size = max([w,h])
        bb_size = max([bb_size, crop_size]) # must be at least crop_size big
        bb_half = bb_size // 2
        ...
        cropped = cropped.resize((crop_size, crop_size), Image.BICUBIC)
        counter += 1
        # save
...
class Loader:
    def __init__(self, opt, path, split, im_w=2048, im_h=1024):
...
if __name__ == '__main__':
    ...
    save_path_cropped_img = './data/cityscapes_imgs_cropped'
    crop_size = 128
    ...
    for split in splits:
        ...
        dataset = Loader(opt, proposed_bbs, split)
        ...
        for i, data in enumerate(dataset):
            ...
            counter = crop_and_save(merged, new_bbs, crop_size, counter, save_path_cropped_img_split)
